...
export async function applyTestSchema(): Promise<void> {
  await env.DB.batch([
    env.DB.prepare('CREATE TABLE IF NOT EXISTS "user" (id TEXT PRIMARY KEY NOT NULL, name TEXT NOT NULL, email TEXT NOT NULL UNIQUE, emailVerified INTEGER NOT NULL DEFAULT 0, image TEXT, createdAt INTEGER NOT NULL, updatedAt INTEGER NOT NULL, units TEXT NOT NULL DEFAULT \'km\' CHECK (units IN (\'km\', \'mi\')))'),
    ...
    env.DB.prepare('CREATE TABLE IF NOT EXISTS maps (id TEXT PRIMARY KEY NOT NULL, owner_id TEXT NOT NULL REFERENCES "user"(id) ON DELETE CASCADE, name TEXT NOT NULL, family_name TEXT, visibility TEXT NOT NULL DEFAULT \'private\' CHECK (visibility IN (\'public\', \'private\')), export_settings TEXT NOT NULL DEFAULT \'{}\', created_at TEXT NOT NULL DEFAULT (datetime(\'now\')), updated_at TEXT NOT NULL DEFAULT (datetime(\'now\')))'),
    env.DB.prepare('CREATE TABLE IF NOT EXISTS stops (id TEXT PRIMARY KEY NOT NULL, map_id TEXT NOT NULL REFERENCES maps(id) ON DELETE CASCADE, position INTEGER NOT NULL, name TEXT NOT NULL, label TEXT, latitude REAL NOT NULL, longitude REAL NOT NULL, icon TEXT, travel_mode TEXT CHECK (travel_mode IS NULL OR travel_mode IN (\'drive\', \'walk\', \'bike\', \'plane\', \'boat\')), created_at TEXT NOT NULL DEFAULT (datetime(\'now\')), type TEXT NOT NULL DEFAULT \'point\' CHECK (type IN (\'point\', \'route\')), dest_name TEXT, dest_latitude REAL, dest_longitude REAL, dest_icon TEXT, route_geometry TEXT)'),
    env.DB.prepare('CREATE TABLE IF NOT EXISTS map_shares (id TEXT PRIMARY KEY NOT NULL, map_id TEXT NOT NULL REFERENCES maps(id) ON DELETE CASCADE, user_id TEXT REFERENCES "user"(id), role TEXT NOT NULL DEFAULT \'viewer\' CHECK (role IN (\'viewer\', \'editor\')), claim_token TEXT UNIQUE, claim_token_expires_at TEXT, created_at TEXT NOT NULL DEFAULT (datetime(\'now\')), UNIQUE(map_id, user_id))'),
    env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_stops_map_id_position ON stops(map_id, position)'),
    ...
    env.DB.prepare('CREATE INDEX IF NOT EXISTS idx_map_shares_map_id ON map_shares(map_id)'),
    env.DB.prepare('CREATE TABLE IF NOT EXISTS orders (id TEXT PRIMARY KEY NOT NULL, map_id TEXT NOT NULL REFERENCES maps(id) ON DELETE RESTRICT, user_id TEXT NOT NULL REFERENCES "user"(id) ON DELETE RESTRICT, product_type TEXT NOT NULL, product_sku TEXT NOT NULL, poster_size TEXT NOT NULL, status TEXT NOT NULL DEFAULT \'pending_payment\' CHECK (status IN (\'pending_payment\', \'paid\', \'pending_render\', \'submitted\', \'in_production\', \'shipped\', \'completed\', \'cancelled\', \'failed\')), stripe_session_id TEXT UNIQUE, prodigi_order_id TEXT, image_url TEXT, shipping_address TEXT, subtotal INTEGER, shipping_cost INTEGER, currency TEXT NOT NULL DEFAULT \'usd\', tracking_url TEXT, discord_notified INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL DEFAULT (datetime(\'now\')), updated_at TEXT NOT NULL DEFAULT (datetime(\'now\')))'),
